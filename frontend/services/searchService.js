import { useStore, updateState, MIN_INTERVAL, API_BASE_URL, itemsPerPage } from '../store.js'
...
async function fetchAllResults(selectedMod) {
  ...
  try {
    ...
    const pagesToFetch = Array.from({ length: totalPages }, (_, i) => i + 1).filter(
      (page) => !(page === currentPage && currentApiResults.length),
    )
...
export async function search(resetPage = false) {
  ...
  if (resetPage) {
    ...
    if (
      store.lastFullSearchKey.split('_')[0] !== query ||
      store.lastFullSearchKey.split('_')[1] !== mode
    ) {
  ...
  if (
    currentModFilter &&
    newAllApiResults.length > 0 &&
    newAllApiResults.length >= store.totalApiMatches
  ) {
    applyModFilter()
    return
  }
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/search?q=${encodeURIComponent(query)}&page=${newPage}&mode=${mode}`,
    )
    const requestEndTime = performance.now()
    updateState({
      searchInfoMessage: `搜索耗时: ${(requestEndTime - requestStartTime).toFixed(0)} 毫秒`,
    })
